...
Log.Debug($"Done Emitting TypeScript interfaces for {ConfigurationName}...");
public string RenderImportStatements()
{
    ...
    localCode.AppendLine($@"import {{ ItemEx, GqlItemEx }} from './_.Sitecore.Override'");
    localCode.AppendLine($@"// @ts-ignore ");
    ...
    foreach (var template in GetBaseTemplates(Templates))
    {
        ...
        if (!foundModules.Contains(nameSpaceAlias))
        {
            ...
            var module = split[1];
            localCode.AppendLine($@"// @ts-ignore ");
            localCode.AppendLine($@"import {{ {shortNameSpaceRoot} as {nameSpaceAlias} }} from ""./{layer}.{module}.model""");
        }
    }
    ...
    foreach (var template in Templates)
    {
        ...
        {
            var typeData = GetCustomFieldItemType(field) ?? new TypeData[] { };

            foreach (var typeDatum in typeData)
            {
                if (!foundModules.Contains(typeDatum.NameSpaceAlias))
                {
                    foundModules.Add(typeDatum.NameSpaceAlias);
                    localCode.AppendLine($@"// @ts-ignore ");
                    localCode.AppendLine($@"import {{ {typeDatum.ShortRootNameSpace} as {typeDatum.NameSpaceAlias} }} from ""./{typeDatum.Layer}.{typeDatum.Module}.model""");
                }
            }
        }
    }
...
public string RenderInterfaceFields(TemplateCodeGenerationMetadata template, bool useJsonValue = false)
{
    ...
    foreach (var field in template.OwnFields)
    {
        var fieldType = GetFieldType(field, useJsonValue);

        if (useJsonValue && !fieldType.StartsWith("{"))
        {
            fieldType = $@"{{
                jsonValue: {fieldType}
            }}";
    ...
    if (!useJsonValue && IncludeChildren(template))
    {
        var itemTypes = GetCustomChildrenItemType(template)?.Select(x => useJsonValue ? $"({x.JssTypeName}Json & GqlItemEx)" : $@"({x.JssTypeName} & ItemEx)")
                                                            .ToList()
                                                             ?? new List<string>() { useJsonValue ? "any" : "Item" };

        var itemType = itemTypes.Count > 1 ? $"({string.Join(" | ", itemTypes)})" : itemTypes.First();
...
public string GetFieldType(TemplateFieldCodeGenerationMetadata field, bool useJsonValue = false)
{

    var itemTypes = GetCustomFieldItemType(field)?.Select(x => useJsonValue ? $"({x.JssTypeName}Json & GqlItemEx)" : $@"({x.JssTypeName} & ItemEx)")
                                                    .ToList()
                                                    ?? new List<string>() { useJsonValue ? "any" : "Item" };
...
{
    return IsOrInheritsFromTemplate(template, "{231BAAFA-38D7-4043-97A3-B16F0E7287BE}");
}